...
把早期对话让 LLM 生成一段摘要，替换掉原文。比如把前 100 轮压缩成"用户偏好清淡食物，预算通常在 50 块以内，曾记录吃过火锅和拉面"这样一段话，保留关键信息但大幅减少 token。
...
每步 Prompt 里都重复一遍主目标，相当于每次都提醒 LLM 你的任务是什么。
优点是简单零成本
缺点是每轮都要重复一遍，白白浪费 token，而且 LLM 还是可能忽略掉

**显式任务状态**

维护一个 scratchpad，记录哪些步骤已完成、哪些还没做。
优点是结构清晰，中途断掉也能从 checkpoint 恢复
缺点是需要额外设计状态管理

**层级 Agent**

上层 Orchestrator 负责记住总目标，下层 SubAgent 只管执行具体子任务，各司其职。
优点是职责清晰，适合复杂长程任务
缺点是架构复杂、调试难度高

**定期自检**

每执行 N 步，让 LLM 自己问自己我现在做的事还在目标范围内吗？
优点是能及时发现偏离
缺点是多了额外的 LLM 调用成本
...
核心思路是：工具在后台跑，用户不用等，结果出来了再通知。三个步骤：
...
本质是LLM调函数，给 LLM 一堆工具，LLM 决定调哪个、传什么参数，结果返回给 LLM 再回复用户。工具是被动的，每次调用都是独立的一次性操作，解决的是LLM怎么调工具这个问题。

优点是生态兼容广，接入成本低，解析简单。
缺点是思考过程和工具调用强制分离，只能做单次查询型操作，没办法连续自主执行多步任务。
...
优点是工具能力强，任务执行完整，关键操作有人工确认保障，不会因为 LLM 理解偏差造成不可逆操作。
缺点是生态相对封闭，重度依赖 Claude 自身能力。

**核心差距**

Function calling 解决的是调一个函数
Claude Code 解决的是完成一个任务，工具调用在 Claude Code 里只是其中一个环节，不是全部。
