...
**1. 天气城市如何获取**
讨论了IP定位、GPS定位、显式传参三种方案，最终选择显式传参。测试题场景下测试用例需要确定性，IP定位在代理环境下容易定位错误，显式传参最稳定可控。

**2. 天气API选型**
先尝试和风天气，免费套餐不包含GeoAPI城市查询接口，返回404。改用wttr.in，完全免费、无需Key、直接用城市名查询。

**3. 要不要做前端**
题目要求终端或网页操作录屏二选一，选择纯终端。核心考察点在Agent Runtime，前端不加分反而费时。

### Prompt 调优心得

在开发过程中，我发现Prompt对Agent的行为影响非常大。初版Prompt容易导致模型直接回答，而不是调用工具。通过不断调整Tool Calling Rules、Context组织方式以及ReAct输出格式，最终实现了能够自主决策、连续调用多个工具并根据Observation继续推理的Agent。这一过程让我体会到，Prompt不仅是提示词，更是Agent行为逻辑的重要组成部分。