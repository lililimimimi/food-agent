...
**优点**：是简单零成本
**缺点**：是每轮都要重复一遍，白白浪费 token，而且 LLM 还是可能忽略掉

**显式任务状态**

维护一个 scratchpad，记录哪些步骤已完成、哪些还没做。
**优点**：是结构清晰，中途断掉也能从 checkpoint 恢复
**缺点**：是需要额外设计状态管理
...
**优点**：是生态兼容广，接入成本低，解析简单。
**缺点**：是思考过程和工具调用强制分离，只能做单次查询型操作，没办法连续自主执行多步任务。
...
**优点**：是工具能力强，任务执行完整，关键操作有人工确认保障，不会因为 LLM 理解偏差造成不可逆操作。
**缺点**：是生态相对封闭，重度依赖 Claude 自身能力。
