...
讨论了三种方案：IP定位、GPS定位、显式传参。最终选择**显式传参**（用户告诉 Agent 城市名），理由是：测试题场景下测试用例需要确定性，IP定位在代理环境下容易定位错误（本地开发环境走了 Clash Verge 代理），显式传参最稳定可控。Agent 在用户未提供城市时会主动追问，这本身也体现了 ReAct "发现缺失信息→主动询问" 的能力。

**2. 天气 API 选型**

先尝试和风天气（QWeather），免费套餐不包含 GeoAPI 城市查询接口，返回 404。改用 **wttr.in**，完全免费、无需 Key、直接用城市名查询，适合 Demo 场景。

**3. 要不要做前端？**

题目要求"终端或网页操作录屏"二选一，最终选择**纯终端**。核心能力（ReAct Loop、工具调用、Session 管理）都在后端，前端不加分反而费时。终端 Trace 输出（Thought/Action/Observation/Final）本身已经很直观。

**4. Session 内存缓存 vs Redis**

天气数据在 Session 级别用内存缓存（Python dict），不引入 Redis。理由：单用户单 Session 短时对话场景不需要分布式缓存，内存缓存足够，Session 结束后自动释放。生产环境扩展多 worker 时可替换为 Redis。

**5. Context 压缩策略**

选择简单的"保留最近 N 条 + 早期摘要"方案，没有用 LLM 生成摘要。理由：题目明确说"复杂的压缩不用在这里实现"，简单策略已满足要求，且测试稳定不会引入额外 API 调用。